...
export function ChildrenList() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const nextChildren = await getChildren();
      setChildren(nextChildren);
      return nextChildren;
    } catch (reason) {
      setError(reason instanceof MemberApiError ? reason : new MemberApiError(0, "자녀 목록을 불러오지 못했습니다."));
      return [];
  ...
  useEffect(() => {
    ...
    const returnFocusElement = deleteTriggerRef.current;
      ...
      <section className="flex min-w-0 flex-col gap-4">
        ...
        {!loading && error && (
          <div className="rounded-2xl border border-red-200 bg-white p-8 text-center dark:border-red-950 dark:bg-[#101419]">
            <AlertTriangle className="mx-auto h-9 w-9 text-red-500" /><h2 className="mt-3 text-lg font-extrabold">{error.status === 403 ? "자녀 목록에 접근할 수 없습니다" : loginRequired ? "로그인이 필요합니다" : "자녀 목록을 불러오지 못했습니다"}</h2><p className="mt-2 text-sm font-medium text-zinc-500">{error.status === 403 ? "이메일 인증 또는 접근 권한을 확인한 뒤 다시 시도해 주세요." : error.message}</p>
            {loginRequired ? <Link href="/auth/login" className="mt-5 inline-flex h-11 items-center rounded-[10px] bg-mint-500 px-5 font-bold text-white">로그인</Link> : <button type="button" onClick={() => void load()} className="mt-5 inline-flex h-11 items-center gap-2 rounded-[10px] border border-zinc-300 px-5 font-bold dark:border-zinc-700"><RefreshCw className="h-4 w-4" />다시 시도</button>}
          </div>
        )}
        {!loading && !error && children.length === 0 && (
          <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-10 text-center dark:border-zinc-700 dark:bg-[#101419]"><UsersRound className="mx-auto h-10 w-10 text-zinc-400" /><h2 className="mt-4 text-xl font-extrabold">등록된 자녀가 없어요</h2><p className="mt-2 text-sm font-medium text-zinc-500 dark:text-zinc-400">자녀를 등록하고 자녀 급식과 알레르기 설정을 관리해 보세요.</p><Link href="/children/new" className="mt-5 inline-flex h-11 items-center rounded-[10px] border border-mint-500 px-5 font-bold text-mint-600 transition-colors hover:bg-mint-50 dark:text-mint-400 dark:hover:bg-mint-950/20">자녀 등록하기</Link></div>
        )}
