...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    SENT: "bg-mint-500/10 text-mint-700 dark:text-mint-300",
    PENDING: "bg-sky-500/10 text-sky-700 dark:text-sky-300",
    SENDING: "bg-sky-500/10 text-sky-700 dark:text-sky-300",
    FAILED: "bg-red-500/10 text-red-700 dark:text-red-300",
    RETRY_PENDING: "bg-amber-500/10 text-amber-700 dark:text-amber-300",
    CANCELED: "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300",
  };
  return <span className={`inline-flex h-7 items-center rounded-[9px] px-2.5 text-xs font-semibold ${styles[status] ?? "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"}`}>{statusLabels[status] ?? status}</span>;
}

function NotificationRow({ item }: { item: NotificationHistoryItem }) {
  const reason = reasonLabels[item.reason] ?? item.reason;
  return <div className="grid gap-3 border-t border-zinc-200 px-6 py-4 text-sm font-semibold dark:border-zinc-800 md:grid-cols-6 md:items-center md:gap-4 md:px-8">
...
export function ChildNotificationHistory({ childId }: { childId: string }) {
  ...
  return <div className="mx-auto flex w-full max-w-[1220px] flex-col gap-4 px-5 pb-12 pt-5">
    ...
    <section aria-labelledby="notification-history-heading" className="overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-[#101419]"><div className="border-b border-zinc-200 px-6 py-5 md:px-8 dark:border-zinc-800"><h2 id="notification-history-heading" className="text-xl font-extrabold tracking-[-0.02em]">알림 발송 이력</h2><p className="mt-1 text-sm font-medium text-zinc-500 dark:text-zinc-400">총 {history.totalCount}건</p></div>{loading ? <div className="flex min-h-48 items-center justify-center text-sm font-bold text-zinc-500"><LoaderCircle className="mr-2 h-5 w-5 animate-spin" /> 페이지를 불러오고 있습니다.</div> : notifications.length ? <><div className="hidden grid-cols-6 gap-4 bg-zinc-50 px-6 py-4 text-sm font-bold text-zinc-500 dark:bg-black/20 md:px-8 md:grid"><span>알림일</span><span>채널</span><span>알림 발생 사유</span><span>발송 상태</span><span>발송 시각</span><span>실패 코드</span></div><div aria-live="polite">{notifications.map((item) => <NotificationRow key={item.notificationId} item={item} />)}</div></> : <div className="flex min-h-48 flex-col items-center justify-center px-6 text-center md:px-8"><CircleDashed className="h-9 w-9 text-zinc-400" /><h3 className="mt-3 font-extrabold">표시할 알림 이력이 없습니다</h3><p className="mt-1 text-sm font-medium text-zinc-500">알림이 발송되면 이곳에서 확인할 수 있어요.</p></div>}{totalPages > 1 ? <nav aria-label="알림 이력 페이지 이동" className="flex items-center justify-center gap-2 border-t border-zinc-200 px-6 py-4 dark:border-zinc-800 md:px-8"><button type="button" aria-label="이전 페이지" disabled={page === 1 || loading} onClick={() => setPage((value) => value - 1)} className="flex h-10 w-10 items-center justify-center rounded-[10px] border border-zinc-300 transition-colors hover:border-mint-500 hover:text-mint-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-mint-500 disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-700 dark:hover:text-mint-300"><ChevronLeft className="h-5 w-5" /></button><span className="min-w-10 text-center text-sm font-extrabold">{page} / {totalPages}</span><button type="button" aria-label="다음 페이지" disabled={page >= totalPages || loading} onClick={() => setPage((value) => value + 1)} className="flex h-10 w-10 items-center justify-center rounded-[10px] border border-zinc-300 transition-colors hover:border-mint-500 hover:text-mint-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-mint-500 disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-700 dark:hover:text-mint-300"><ChevronRight className="h-5 w-5" /></button></nav> : null}</section>
  </div>;
}
