...
function SummaryRow({ title, description, primaryLabel, primaryCount, secondaryLabel, secondaryCount, href, linkLabel }: {
  title: string;
  description: string;
  primaryLabel: string;
...
export function AdminDashboard() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1220px] flex-col gap-4 px-5 pb-12 pt-5">
      ...
      {summary && <section aria-label="서비스 운영 현황" aria-busy={loading} className={`${panel} px-6 py-3 md:px-8 md:py-4`}><div className="border-b border-zinc-200 py-4 text-sm font-medium text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">집계 시각: <time>{summary.generatedAt ? new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium", timeStyle: "short", hour12: false }).format(new Date(summary.generatedAt)) : "미제공"}</time></div><div className="divide-y divide-zinc-200 dark:divide-zinc-800"><SummaryRow title="급식 수집" description="수집 작업의 현재 처리 현황입니다." primaryLabel="수집 실패" primaryCount={summary.collection?.failedCount} secondaryLabel="수집 대기" secondaryCount={summary.collection?.pendingCount} href="/admin/collection-failures" linkLabel="수집 실패 보기" /><SummaryRow title="알레르기 라벨링" description="알레르기 분석 작업의 처리 현황입니다." primaryLabel="라벨링 실패" primaryCount={summary.labeling?.labelingFailedCount} secondaryLabel="처리 대기" secondaryCount={summary.labeling?.pendingCount} href="/admin/collection-failures" linkLabel="수집 현황 보기" /><SummaryRow title="이벤트 발행" description="outbox 이벤트의 발행 처리 현황입니다." primaryLabel="발행 완료" primaryCount={summary.outbox?.publishedCount} secondaryLabel="발행 대기" secondaryCount={summary.outbox?.pendingCount} href="/admin/notification-dlq-events" linkLabel="운영 화면 보기" /><SummaryRow title="DLQ 이벤트" description="자동 재시도 후 재처리를 기다리는 이벤트입니다." primaryLabel="재처리 대기" primaryCount={summary.dlq?.pendingCount} secondaryLabel="누적 재처리" secondaryCount={summary.dlq?.reprocessedCount} href="/admin/notification-dlq-events" linkLabel="DLQ 이벤트 보기" /><SummaryRow title="알림 발송" description="발송에 실패했거나 재시도 중인 알림입니다." primaryLabel="발송 실패" primaryCount={summary.notifications?.failedCount} secondaryLabel="재시도 대기" secondaryCount={summary.notifications?.retryPendingCount} href="/admin/notification-failures" linkLabel="실패 알림 보기" /></div></section>}
    </div>
  );
}
